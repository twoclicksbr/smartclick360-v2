<?php

namespace App\Http\Controllers\Tenant;

use App\Http\Controllers\Controller;
use App\Models\Tenant\Unit;
use Illuminate\Http\Request;

class UnitsController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index(Request $request, string $slug, string $module)
    {
        $units = Unit::orderBy('order')->get();

        if ($request->ajax()) {
            return response()->json([
                'success' => true,
                'data' => $units,
            ]);
        }

        return view('tenant.pages.units.index', compact('units'));
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create(string $slug, string $module)
    {
        return view('tenant.pages.units.create');
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(Request $request, string $slug, string $module)
    {
        $validated = $request->validate([
            'name' => 'required|string|max:255',
            'abbreviation' => 'required|string|max:10',
            'decimal_places' => 'required|integer|min:0|max:8',
        ], [
            'name.required' => 'O nome é obrigatório',
            'abbreviation.required' => 'A abreviação é obrigatória',
            'abbreviation.max' => 'A abreviação deve ter no máximo 10 caracteres',
            'decimal_places.required' => 'As casas decimais são obrigatórias',
            'decimal_places.integer' => 'As casas decimais devem ser um número inteiro',
            'decimal_places.min' => 'As casas decimais devem ser no mínimo 0',
            'decimal_places.max' => 'As casas decimais devem ser no máximo 8',
        ]);

        $unit = Unit::create([
            'name' => $validated['name'],
            'abbreviation' => $validated['abbreviation'],
            'decimal_places' => $validated['decimal_places'],
            'order' => Unit::max('order') + 1,
            'status' => $request->input('status', 1) == 1,
        ]);

        if ($request->ajax()) {
            return response()->json([
                'success' => true,
                'message' => 'Unidade criada com sucesso!',
                'data' => $unit,
            ]);
        }

        return redirect()->route('units.index', ['slug' => $slug])
            ->with('success', 'Unidade criada com sucesso!');
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(string $slug, string $module, string $code)
    {
        $id = decodeId($code);
        $unit = Unit::findOrFail($id);

        if (request()->ajax()) {
            return response()->json([
                'success' => true,
                'data' => $unit,
            ]);
        }

        return view('tenant.pages.units.edit', compact('unit'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, string $slug, string $module, string $code)
    {
        $id = decodeId($code);
        $unit = Unit::findOrFail($id);

        $validated = $request->validate([
            'name' => 'required|string|max:255',
            'abbreviation' => 'required|string|max:10',
            'decimal_places' => 'required|integer|min:0|max:8',
        ], [
            'name.required' => 'O nome é obrigatório',
            'abbreviation.required' => 'A abreviação é obrigatória',
            'abbreviation.max' => 'A abreviação deve ter no máximo 10 caracteres',
            'decimal_places.required' => 'As casas decimais são obrigatórias',
            'decimal_places.integer' => 'As casas decimais devem ser um número inteiro',
            'decimal_places.min' => 'As casas decimais devem ser no mínimo 0',
            'decimal_places.max' => 'As casas decimais devem ser no máximo 8',
        ]);

        $unit->update([
            'name' => $validated['name'],
            'abbreviation' => $validated['abbreviation'],
            'decimal_places' => $validated['decimal_places'],
            'status' => $request->input('status', 1) == 1,
        ]);

        if ($request->ajax()) {
            return response()->json([
                'success' => true,
                'message' => 'Unidade atualizada com sucesso!',
                'data' => $unit,
            ]);
        }

        return redirect()->route('units.index', ['slug' => $slug])
            ->with('success', 'Unidade atualizada com sucesso!');
    }

    /**
     * Soft delete the specified resource.
     */
    public function destroy(string $slug, string $module, string $code)
    {
        $id = decodeId($code);
        $unit = Unit::findOrFail($id);

        $unit->delete();

        if (request()->ajax()) {
            return response()->json([
                'success' => true,
                'message' => 'Unidade excluída com sucesso!',
            ]);
        }

        return redirect()->route('units.index', ['slug' => $slug])
            ->with('success', 'Unidade excluída com sucesso!');
    }

    /**
     * Restore a soft deleted resource.
     */
    public function restore(string $slug, string $module, string $code)
    {
        $id = decodeId($code);
        $unit = Unit::withTrashed()->findOrFail($id);

        $unit->restore();

        if (request()->ajax()) {
            return response()->json([
                'success' => true,
                'message' => 'Unidade restaurada com sucesso!',
                'data' => $unit,
            ]);
        }

        return redirect()->route('units.index', ['slug' => $slug])
            ->with('success', 'Unidade restaurada com sucesso!');
    }

    /**
     * Reorder items via drag and drop
     */
    public function reorder(Request $request, string $slug, string $module)
    {
        $validated = $request->validate([
            'items' => 'required|array',
            'items.*.id' => 'required|exists:units,id',
            'items.*.order' => 'required|integer',
        ]);

        foreach ($validated['items'] as $item) {
            Unit::where('id', $item['id'])->update(['order' => $item['order']]);
        }

        if ($request->ajax()) {
            return response()->json([
                'success' => true,
                'message' => 'Ordem atualizada com sucesso!',
            ]);
        }

        return redirect()->route('units.index', ['slug' => $slug])
            ->with('success', 'Ordem atualizada com sucesso!');
    }
}
