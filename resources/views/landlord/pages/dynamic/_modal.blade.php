                        ...
                        @foreach($fieldsWithUi as $field)
                            ...
                            <div class="{{ $field->grid_col ?? 'col-md-12' }} field-create {{ $field->visible_create ? '' : 'd-none' }} field-edit {{ $field->visible_edit ? '' : 'd-none' }}"
                                ...
                                @switch($field->component)
                                    ...
                                    @case('select_module')
                                        @php
                                            $fkRecords = \Illuminate\Support\Facades\DB::connection($field->connection ?? 'landlord')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    modal.addEventListener('show.bs.modal', function(e) {
        // Só reseta se não estiver em modo edição (verificando formMethod ao invés de relatedTarget)
    ...
    // Botão editar da listagem: redireciona para página de detalhes (show)
    document.querySelectorAll('.btn-edit').forEach(btn => {
        btn.addEventListener('click', function(e) {
            e.preventDefault();
            const code = this.dataset.code;
            window.location.href = `/${moduleSlug}/${code}`;
        });
    });
