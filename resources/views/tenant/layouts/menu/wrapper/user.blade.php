@php
    ...
    $subscription = $tenant ? $tenant->subscriptions()->where('status', 'active')->orWhere('status', 'trial')->latest()->first() : null;
    $planName = $subscription ? $subscription->plan->name : 'Free';

    // Define a cor do badge baseado no plano
    $badgeColor = match($planName) {
        'Starter' => 'light-primary',
        'Professional' => 'light-success',
        'Enterprise' => 'light-warning',
            ...
            <div class="menu-content d-flex align-items-center px-3">
                ...
                <div class="d-flex flex-column">
                    <div class="fw-bold d-flex align-items-center fs-5">{{ $fullName }}
                        <span class="badge badge-{{ $badgeColor }} fw-bold fs-8 px-2 py-1 ms-2">{{ $planName }}</span>
                    </div>
